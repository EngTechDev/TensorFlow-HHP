...
#   Eager execution must be disabled to use placeholders
...
#   Define number of training samples, 0.7 = 70%. Taking the first 70% since values are randomized
...
tf_house_size = tf.compat.v1.placeholder(tf.float32, name="house_size")
tf_price = tf.compat.v1.placeholder(tf.float32, name="price")


#   Define the variable holding the size_factor and price set during training.
#Initialize to random values based on normal distribution.
tf_size_factor = tf.Variable(np.random.randn(), name="size_factor")
tf_price_offset = tf.Variable(np.random.randn(), name="price_offset")

# 2.    Define the operations for predicting values
#       Notice the use of the tensorflow add and multiply
#       AND the tensorflow methods understand how to deal methods
##############################################################################################################
#@tf.function
#def forward(tf_size_factor):
#    return tf_house_size * tf_size_factor + tf_price_offset
...
optimizer = tf.optimizers.SGD(name='SGD').minimize(tf_cost, var_list=[tf_size_factor, tf_price_offset])
...
with tf.compat.v1.Session() as sess:
    ...
    #   get values used to normalize data: denormalize data back to its original scale
    ...
    #
    #   Plot another graph that animates how Gradient Descent sequentially adjusted size_factor and price_offset to
