...
def show_training_image(index):
    img_label = str(label[index]) + ' (' + class_names[label[index]] + ')'
    plt.figure()
    plt.title('Image Label ' + img_label)
    plt.imshow(images[index], cmap='gray')    # data is grayscale, but displays in color with out cmap='gray'
    plt.colorbar()
    plt.show()
...
train_hist = ToyMod_Model.fit(train_generator, y=None, epochs=10,
...
outputs = tf.keras.layers.Dense(6, activation='softmax')(drop)
ToyMod_Model = tf.keras.Model(inputs=inputs, outputs=outputs)
...
ToyMod_Model.fit(train_generator, y=None, epochs=10,
                            validation_data=validation_generator,
                            callbacks=[tensorboard_callback])
